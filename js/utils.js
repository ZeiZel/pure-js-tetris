...
export const isValidPosition = (tetromino, tetrominoCol, tetrominoRow, playArea) => {
...
// функция ускорения падения вниз
export const rapidFallOnDown = (tetromino, playArea, placeTetromino) => {
	const row = tetromino.row + 1;

	if (!isValidPosition(tetromino.matrix, row, tetromino.col, playArea)) {
		tetromino.row = row - 1;
		placeTetromino();
		return;
	}
...
export const rotateOnClickUp = (tetromino, playArea) => {
	...
	// проверяет, можем ли мы повернуть тетромино возле границ канваса
	if (!isValidPosition(matrix, tetromino.row, tetromino.col, playArea)) {
...
export const moveOnClickRight = (tetromino, playArea) => {
	...
	if (isValidPosition(tetromino.matrix, tetromino.row, col, playArea)) {
		tetromino.col = col;
	}
};
